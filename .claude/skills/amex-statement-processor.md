...
後述のプロンプト（勘定科目推定ロジック・消費税率判定ロジック・仕訳台帳19カラム共通フォーマット + cardholder）を適用して、各取引を仕訳データに変換する。
...
- ほとんどの取引はこちらに該当
- クレカ明細にはインボイス番号が記載されないため、デフォルトは「非インボイス」とする。Phase 2 でレシート突合後に「インボイス」へ変更可能
...
**注意事項**: 海外SaaS事業者が日本で適格請求書発行事業者として登録済みの場合は「課税仕入10%（非インボイス）」となる可能性がある。判断が困難な場合は「課対仕入（リバースチャージ）」を適用し、`memo` に「税区分要確認」と記載する

#### その他注意事項

- コンビニ・スーパーは食品購入が主と推定し軽減税率8%を適用する。ただし、明らかに食品以外（雑誌、タバコ等）とわかる場合は10%とする
- 飲食店でのイートインは標準税率10%、テイクアウト・デリバリーは軽減税率8%
- 海外のコンビニ・飲食チェーン（7-ELEVEN、MCDONALD'S等）でも外貨建ての場合は「対象外」とする
- 判断が困難な場合は標準税率10%（非インボイス）を適用し、`memo` に「税率要確認」と記載する

### 仕訳台帳フォーマット（19カラム共通 + cardholder）

各取引を以下の19カラム（共通フォーマット）+ cardholder（Amex固有）で構造化する。
...
| G | 借方税区分 | `debit_tax_category` | 「課税仕入10%（非インボイス）」or「課税仕入8%（軽減・非インボイス）」or「対象外」or「課対仕入（リバースチャージ）」 |
...
**transactions[] の各要素**:
- 19カラム共通仕訳データ + `cardholder`（Amex固有）+ `status` フィールド
