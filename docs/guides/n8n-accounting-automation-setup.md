...
- [Airレジ日報自動作成バッチ](#airレジ日報自動作成バッチ)

---

## 概要

このドキュメントでは、以下のn8nワークフローのセットアップ方法を説明します：

### 1. LINE経理自動化ワークフロー

LINE公式アカウント経由で以下の処理を自動化します：
...
Airレジの売上CSVから日報を自動生成し、以下を実現します：

1. **毎日深夜バッチ実行**（前日分の売上データを集計）
2. **Googleスプレッドシートへの記録**（日報シートに自動追記）
3. **Mattermost通知**（売上サマリーを自動送信）
...
Airレジの売上CSV（会計明細）を自動取得し、日報を作成するバッチワークフローです。

**主な機能:**
- 毎日深夜2時に自動実行
- Google Driveから前日分の会計明細CSVを取得
- Shift-JIS → UTF-8 エンコーディング変換
- 取引Noごとにグループ化して集計
- 基本指標・時間帯別・支払方法別の分析
- Googleスプレッドシートに日報を記録
- Mattermostに売上サマリーを通知

**ワークフローファイル:**
- `n8n/workflows/air-register-daily-report-batch.json`
...
各顧客用のスプレッドシートに、`日報` シートを作成します。

**シート名:** `日報`

| A: 日付 | B: 曜日 | C: 売上(税込) | D: 売上(税抜) | E: 消費税 | F: 客数 | G: 客単価 | H: 取引件数 | I: ランチ売上 | J: ディナー売上 | K: 現金 | L: クレジットカード | M: 電子マネー |
|---------|---------|---------------|---------------|-----------|---------|-----------|-------------|---------------|----------------|---------|-------------------|--------------|
| 2026-01-20 | 月 | 150,000 | 136,364 | 13,636 | 45 | 3,333 | 38 | 60,000 | 80,000 | 50,000 | 90,000 | 10,000 |

**ヘッダー行（1行目）を必ず作成してください。**

#### 2. Google Driveフォルダ構成

顧客別のGoogle Driveフォルダに、Airレジの会計明細CSVをアップロードします。

```
Google Drive
└── 顧客A フォルダ (drive_folder_id: 1A2B3C...)
    ├── 会計明細_202601200500-202601210459.csv  ← 最新
    ├── 会計明細_202601190500-202601200459.csv
    └── 会計明細_202601180500-202601190459.csv
```

**ファイル名パターン:**
```
会計明細_YYYYMMDDHHmm-YYYYMMDDHHmm.csv
```

例: `会計明細_202601200500-202601210459.csv`
- 開始: 2026年1月20日 5:00
- 終了: 2026年1月21日 4:59
- 24時間分のデータ

#### 3. Master_User_Configの設定

Master_User_Configシートに、以下の列が設定されていることを確認してください：
...
| `sheet_id` | スプレッドシートID | ✅ |
| `mattermost_webhook_url` | Mattermost WebhookURL | ✅ |
...
2. 左メニューから「Workflows」を選択
3. 右上の「Import from File」をクリック
4. `n8n/workflows/air-register-daily-report-batch.json` を選択
5. インポート完了後、ワークフローが開きます

#### 5. Credentialsの紐付け

以下のノードでCredentialsを設定：

- 「全クライアント取得」ノード → `gsheet-oauth`
- 「AirレジCSV検索」ノード → `gdrive-oauth`
- 「CSVダウンロード」ノード → `gdrive-oauth`
- 「日報をスプレッドシートに記録」ノード → `gsheet-oauth`

#### 6. Master_User_Config シートIDの設定

「全クライアント取得」ノードをクリックし、以下を設定：
...
3. 以下を確認：
   - エラーなく実行完了するか
   - 各ノードが緑色（成功）になっているか
   - Googleスプレッドシート「日報」に行が追加されているか
   - Mattermostに通知が届くか

#### Test 2: CSVファイル名パターンテスト

1. Google Driveに前日分のCSVをアップロード
   - ファイル名: `会計明細_YYYYMMDDHHmm-YYYYMMDDHHmm.csv`
2. ワークフローを手動実行
3. 以下を確認：
   - CSVが正しく検索・取得されるか
   - エンコーディング変換が正しく動作するか
...
1. サンプルCSVで集計結果を確認：
   ...
   - 客数が正しいか
   - 客単価の計算が正しいか（売上 ÷ 客数）
   - 時間帯別集計が正しいか（ランチ 11:00-14:00、ディナー 17:00-22:00）

#### Test 4: Mattermost通知の内容確認

通知メッセージに以下が含まれているか確認：
- ✅ 顧客名
- ✅ 日付・曜日
- ✅ 売上（税込）
- ✅ 客数・客単価
...
**原因:** ファイル名パターンが一致しない、またはフォルダIDが間違っている

**解決策:**
1. Google Driveのファイル名を確認
   - パターン: `会計明細_YYYYMMDDHHmm-YYYYMMDDHHmm.csv`
2. Master_User_Configの `drive_folder_id` が正しいか確認
3. n8nの実行ログで検索結果を確認

#### 問題2: Shift-JIS変換が失敗する

**原因:** iconv-liteライブラリが利用できない

**解決策:**
1. n8nのCodeノードで `iconv-lite` が利用可能か確認
2. 利用できない場合、以下の代替案を検討：
   - Execute Commandノードで `iconv` コマンド使用
   - Google Apps Scriptで事前変換

#### 問題3: スプレッドシートに記録されない

**原因:** シート名が一致しない、またはヘッダー行がない

**解決策:**
1. シート名が **完全一致** で `日報` になっているか確認（全角・半角注意）
2. ヘッダー行（1行目）が存在するか確認
3. Googleスプレッドシートの権限を確認（n8nサービスアカウントに編集権限）

#### 問題4: Mattermost通知が届かない

**原因:** Webhook URLが間違っている、またはチャンネルの権限がない

**解決策:**
1. Master_User_Configの `mattermost_webhook_url` が正しいか確認
2. Mattermostのチャンネル設定でIncoming Webhooksが有効か確認
3. n8nの実行ログでHTTPレスポンスコードを確認（200 OKか）
...
| 項目 | 説明 | 計算方法 |
|------|------|----------|
| 売上(税込) | 総売上（税込） | 全取引の合計金額 |
| 売上(税抜) | 総売上（税抜） | 全取引の小計合計 |
| 消費税 | 消費税額 | 全取引の消費税合計 |
| 客数 | 来店客数 | 全取引の人数合計 |
| 客単価 | 客単価 | 売上(税込) ÷ 客数 |
| 取引件数 | 取引件数 | 取引Noの総数 |
