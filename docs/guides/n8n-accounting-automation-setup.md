...
Google Drive上の顧客フォルダに配置されたアメックス明細PDFを手動トリガーで解析し、仕訳台帳に自動登録するワークフローです。
...
#### FR-1: PDF取得・読み込み
- Google Driveフォルダから未処理PDF（`_processed`なし）を自動取得
- 複数PDFがある場合は順次処理
...
#### FR-7: 処理済みマーキング
- 処理完了後、PDFファイル名を変更
  - 例: `amex_202501.pdf` → `amex_202501_processed.pdf`

### テスト方法

#### Test 1: PDF解析テスト

1. サンプルアメックス明細PDFをGoogle Driveフォルダに配置
2. ワークフローを手動実行
3. 以下を確認:
   - 全取引が正しく抽出される
   - 勘定科目が自動推定される
