...
| `line-image-handler.json` | 子ワークフロー（画像OCR処理、ナレッジベース統合） | Inactive（親から呼び出し） |
| `knowledge-base-learning-batch.json` | バッチ処理（ナレッジベース学習、毎日深夜2時実行） | Active（スケジュール実行） |
| `line-accounting-automation.json` | mainブランチの画像処理ワークフロー（ナレッジベース統合） | Active（現在使用中） |
...
#### 2-2. メインワークフローのインポート（現在使用中）

1. n8n管理画面で、再度「Import from File」をクリック
2. `n8n/workflows/line-accounting-automation.json` を選択
3. インポート完了後、ワークフローが開きます

**注**: このワークフローは現在mainブランチで使用中のナレッジベース統合版です。親-子アーキテクチャへの移行完了後は、このワークフローを無効化します。

#### 2-3. 学習バッチワークフローのインポート
...
#### 5-2. メインワークフロー（LINE経理自動化）の有効化（現在使用中）

1. `LINE経理自動化` ワークフローを開く
2. n8n画面右上の「Active」トグルを **ON** に設定
3. 「Save」をクリック

**注**: 親-子アーキテクチャへの移行完了後は、このワークフローを無効化し、Step 5-1の `line-webhook-router` のみを有効化します。移行期間中は、両方のワークフローを並行稼働させることも可能です（ただし、同じWebhook URLを使用する場合は、どちらか一方のみを有効化してください）。
