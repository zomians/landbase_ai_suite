...
### Iteration 2: 予約予測ダッシュボード

**ゴール**: ホテル・飲食店向けに、過去データから予約数を予測してダッシュボード表示
...
#### 必要となるコア機能の拡充
- ⚙️ PostgreSQL データ分析基盤
- ⚙️ Rails バックグラウンドジョブ（Sidekiq）
- ⚙️ Next.js または Rails View でのダッシュボード
- ⚙️ 認証・認可システム（クライアント毎のデータ隔離）

#### 成果物
- 予測モデル（Python/scikit-learn または Claude Code Interpreter）
- ダッシュボード UI
- データパイプライン
