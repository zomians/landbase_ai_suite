...
export default class extends Controller {
  ...
  async submit(event) {
    ...
    try {
      const response = await fetch("/api/v1/invoices/process_statement", {
        method: "POST",
        body: formData,
        headers: {
      ...
      if (response.status === 409 && data.duplicate) {
        this.hideLoading()
        this.submitButtonTarget.disabled = false
        if (confirm("この請求書は既に処理済みです。再処理しますか？")) {
      ...
      if (!response.ok) {
        this.showError(data.error || "請求書の処理に失敗しました。")
        this.hideLoading()
        this.submitButtonTarget.disabled = false
        return
      }
    ...
    for (let i = 0; i < MAX_POLLS; i++) {
      ...
      try {
        const response = await fetch(
          `/api/v1/invoices/${batchId}/status?client_code=${encodeURIComponent(clientCode)}`
        )
        const data = await response.json()
        ...
        if (data.status === "failed") {
          this.showError(data.error_message || "請求書の処理に失敗しました。")
          return
        }
      } catch (error) {
  ...
  showResult(data, clientCode) {
    ...
    html += `<a href="/journal_entries?client_code=${encodeURIComponent(clientCode)}&source_type=invoice" class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-colors">仕訳一覧を確認</a>`
