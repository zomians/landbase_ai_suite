...
export default class extends Controller {
  ...
  async submit(event) {
    ...
    try {
      const response = await fetch("/api/v1/bank_statements/process_statement", {
        method: "POST",
        body: formData,
        headers: {
    ...
    for (let i = 0; i < MAX_POLLS; i++) {
      ...
      try {
        const response = await fetch(
          `/api/v1/bank_statements/${batchId}/status?client_code=${encodeURIComponent(clientCode)}`
        )
        const data = await response.json()
  ...
  showResult(data, clientCode) {
    ...
    html += `<a href="/journal_entries?client_code=${encodeURIComponent(clientCode)}&source_type=bank" class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-colors">仕訳一覧を確認</a>`
